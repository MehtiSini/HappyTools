...
namespace HappyTools.Utilities.Extensions.HttpClient.CompressionHandler
{
    ...
    public class ClientCompressionHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                await DecompressResponse(response);
            }
        ...
        public async Task CompressRequest(HttpRequestMessage request)
        {
            // As per RFC2616.14.3:
            ...
            if (compressor != null)
            {
                try
                {
                    ...
                    else if (contentSizeThreshold > 0 && (request.Content.Headers.ContentLength == null || request.Content.Headers.ContentLength >= contentSizeThreshold))
                    {
                        request.Content = new CompressedContent(request.Content, compressor);
                    }
                }
        ...
        /// <returns>An async void.</returns>
        public async Task DecompressResponse(HttpResponseMessage response)
        {
            ...
            if (encoding != null)
            {
                ...
                if (compressor != null)
                {
                    try
                    {
                        response.Content = await httpContentOperations.DecompressContent(response.Content, compressor).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
