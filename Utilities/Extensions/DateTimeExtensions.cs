...
namespace HappyTools.Utilities.Extensions
{
    public static class DateTimeExtensions
    {
        ...
        {
            return dateTime.GetMillisecondsSince1970();
        }
        ...
        {
            return dateTime.GetWeekOfYear(CultureInfo.DefaultThreadCurrentUICulture);
        }
      ...
      public  static DateTime ToGeorgian(string persianDateTime)
        {
            ...
            int month = int.Parse(dateParts[1]); // Month
            int day = int.Parse(dateParts[0]);   // Day

            int hour = int.Parse(timeParts[0]);  // Hour
            int minute = int.Parse(timeParts[1]); // Minute
            int second = int.Parse(timeParts[2]); // Second

            if (parts.Length > 2 && parts[2] == "PM" && hour < 12)
            {
                hour += 12; // Convert PM to 24-hour format
            }
            else if (parts.Length > 2 && parts[2] == "AM" && hour == 12)
            {
        ...
        {
            return date.ToFriendlyDateString(CultureInfo.DefaultThreadCurrentUICulture);
        }
