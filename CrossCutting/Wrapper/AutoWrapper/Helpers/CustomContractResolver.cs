...
using HappyTools.CrossCutting.Wrapper.AutoWrapper;
...
namespace HappyTools.CrossCutting.Wrapper.AutoWrapper.Helpers
{
    public class CustomContractResolver<T> : DefaultContractResolver
    {
        ...
        {
            var resolved = _propertyMappings.TryGetValue(propertyName, out var resolvedName);
        ...
        {
            foreach (var propertyInfo in classType.GetProperties(BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance))
            {
